...
$EXE_URL = "$BASE_URL/arc-windows-$ARCH.exe"
...
Write-Host "⬇️ Descargando Archetype para Windows ($ARCH)..."
try {
    Invoke-WebRequest -Uri $EXE_URL -OutFile "$INSTALL_DIR\arc.exe"
...
try {
    ...
    Write-Host "✅ Archetype ha sido agregado al PATH"
} catch {
    Write-Host "⚠️ No se pudo agregar Archetype al PATH: $_"
    Write-Host "Por favor, agrégalo manualmente: $INSTALL_DIR"
}

Write-Host "✨ Instalación completada. Reinicia tu terminal y prueba con: arc --version"