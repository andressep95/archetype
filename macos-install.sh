...
if ! file "$INSTALL_DIR/arc" | grep -q "executable"; then
...
echo 'export PATH="$HOME/.arc:$PATH"' >> "$HOME/.zshrc"
echo 'export PATH="$HOME/.arc:$PATH"' >> "$HOME/.bash_profile"

# Recargar .zshrc y .bash_profile para la sesión actual
source "$HOME/.zshrc"
source "$HOME/.bash_profile"

log "Instalación completada. Prueba ejecutando: arc --version"